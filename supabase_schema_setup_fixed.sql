-- =====================================================
-- Supabase Database Schema Setup - FIXED VERSION
...
DROP TABLE IF EXISTS meta_tokens CASCADE;
...
-- Drop views
DROP VIEW IF EXISTS enhanced_call_analytics CASCADE;
DROP VIEW IF EXISTS users_with_clerk CASCADE;

-- Drop functions
DROP FUNCTION IF EXISTS calculate_close_rate(UUID, DATE, DATE) CASCADE;
DROP FUNCTION IF EXISTS calculate_show_rate(UUID, DATE, DATE) CASCADE;
DROP FUNCTION IF EXISTS calculate_aov(UUID, DATE, DATE) CASCADE;
DROP FUNCTION IF EXISTS sync_clerk_user(VARCHAR, VARCHAR, VARCHAR, JSONB) CASCADE;
...
-- =====================================================
-- 5. CREATE CALLS TABLE (Enhanced Call Logging)
-- =====================================================
...
-- =====================================================
-- 7. CREATE META TOKENS TABLE (for Meta API integration)
-- =====================================================
...
CREATE INDEX idx_calls_closer_name ON calls(closer_first_name, closer_last_name);
CREATE INDEX idx_calls_source_of_set_appointment ON calls(source_of_set_appointment);
CREATE INDEX idx_calls_enhanced_outcome ON calls(enhanced_call_outcome);
CREATE INDEX idx_calls_initial_payment_date ON calls(initial_payment_collected_on);
CREATE INDEX idx_calls_customer_email ON calls(customer_email);
CREATE INDEX idx_calls_setter_name ON calls(setter_first_name, setter_last_name);
CREATE INDEX idx_calls_cash_collected ON calls(cash_collected_upfront);
CREATE INDEX idx_calls_total_owed ON calls(total_amount_owed);
CREATE INDEX idx_calls_lead_source ON calls(lead_source);

-- Composite indexes for common queries
CREATE INDEX idx_calls_client_user_date ON calls(client_id, user_id, created_at);
CREATE INDEX idx_calls_status_outcome ON calls(status, outcome);
CREATE INDEX idx_calls_enhanced_outcome_lead_source ON calls(enhanced_call_outcome, lead_source);
...
CREATE INDEX idx_meta_tokens_client_id ON meta_tokens(client_id);
CREATE INDEX idx_meta_tokens_user_id ON meta_tokens(user_id);
CREATE INDEX idx_meta_tokens_is_active ON meta_tokens(is_active);
...
CREATE VIEW enhanced_call_analytics AS
SELECT 
    c.client_id,
    c.user_id,
...
CREATE VIEW users_with_clerk AS
SELECT 
    u.id,
    u.client_id,
    u.clerk_user_id,
...
CREATE FUNCTION calculate_close_rate(
    p_client_id UUID,
...
CREATE FUNCTION calculate_show_rate(
    p_client_id UUID,
    p_date_from DATE DEFAULT NULL,
    p_date_to DATE DEFAULT NULL
) RETURNS DECIMAL(5,2) AS $$
...
CREATE FUNCTION sync_clerk_user(
