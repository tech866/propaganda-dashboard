...
// GET /api/metrics/enhanced/team - Get team performance metrics
const getTeamMetrics = withErrorHandling(async (request: NextRequest, user: User) => {
  ...
  const metrics = await EnhancedMetricsService.getTeamPerformance(
    clientId,
    dateFrom,
    dateTo
  );
...
export const GET = withAuth(getTeamMetrics);
