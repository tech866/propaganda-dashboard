...
// GET /api/metrics/enhanced/lead-source - Get lead source performance metrics
const getLeadSourceMetrics = withErrorHandling(async (request: NextRequest, user: User) => {
  ...
  const metrics = await EnhancedMetricsService.getLeadSourcePerformance(
    clientId,
    dateFrom,
    dateTo
  );
...
export const GET = withAuth(getLeadSourceMetrics);
