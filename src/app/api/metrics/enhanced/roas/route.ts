import { NextRequest, NextResponse } from 'next/server';
import { withAuth, User } from '@/middleware/auth';
import { canAccessClient } from '@/middleware/utils';
import { 
  createValidationError, 
  createAuthorizationError, 
  withErrorHandling 
} from '@/middleware/errors';
import { EnhancedMetricsService } from '@/lib/services/enhancedMetricsService';

// GET /api/metrics/enhanced/roas - Get ROAS metrics
const getROASMetrics = withErrorHandling(async (request: NextRequest, user: User) => {
  const { searchParams } = new URL(request.url);
  
  const clientId = searchParams.get('clientId') || user.clientId;
  const adSpend = searchParams.get('adSpend');
  const dateFrom = searchParams.get('dateFrom') || undefined;
  const dateTo = searchParams.get('dateTo') || undefined;

  // Validate required adSpend parameter
  if (!adSpend || isNaN(parseFloat(adSpend))) {
    throw createValidationError('Ad spend parameter is required and must be a valid number');
  }

  const adSpendValue = parseFloat(adSpend);
  if (adSpendValue < 0) {
    throw createValidationError('Ad spend must be a non-negative number');
  }

  // Validate client access
  if (!canAccessClient(user, clientId)) {
    throw createAuthorizationError('Access denied to client data');
  }

  // Get ROAS metrics
  const metrics = await EnhancedMetricsService.getROASMetrics(
    clientId,
    adSpendValue,
    dateFrom,
    dateTo
  );

  return NextResponse.json(
    {
      success: true,
      data: metrics,
      filters: { clientId, adSpend: adSpendValue, dateFrom, dateTo },
      user: { id: user.id, role: user.role, clientId: user.clientId }
    },
    { status: 200 }
  );
});

// Export the protected handler
export const GET = withAuth(getROASMetrics);
